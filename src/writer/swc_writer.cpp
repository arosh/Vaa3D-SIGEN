...
static void write_rec(std::ostream &os, const neuron_node *node, const int parent_id) {
  ...
  CHECK_NE(-1, type_id);
  os << node->id_ << ' ' << type_id << ' ' << node->gx_ << ' ' << node->gy_ << ' ' << node->gz_
     << ' ' << node->radius_ << ' ' << parent_id << std::endl;
...
void swc_writer::write(std::ostream &os, const neuron &neu) { write_rec(os, neu.root_, -1); }
